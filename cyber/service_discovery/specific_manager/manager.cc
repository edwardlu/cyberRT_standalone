...
bool Manager::CreatePublisher(RtpsParticipant* participant) {
	...
	RETURN_VAL_IF(!AttributesFiller::FillInPubAttr(channel_name_, QosProfileConf::QOS_PROFILE_TOPO_CHANGE, &pub_attr), false);
...
bool Manager::CreateSubscriber(RtpsParticipant* participant) {
	...
	RETURN_VAL_IF(!AttributesFiller::FillInSubAttr(channel_name_, QosProfileConf::QOS_PROFILE_TOPO_CHANGE, &sub_attr), false);
...
void Manager::OnRemoteChange(const std::string& msg_str) {
	...
	ChangeMsg msg;
#ifdef PROTBUF_VERSION_3_19
	RETURN_IF(!message::ParseFromString(msg_str, &msg));
#else //3.3
	message::ParseFromString(msg_str, &msg);
#endif
	/*Node join and Channel join */
