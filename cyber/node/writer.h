...
private:
	...
	//pointers to HYBIRD/INTER/SHM transmitter
	//inited by Init() 
...
void Writer<MessageT>::JoinTheTopology() {
	// add listener
	change_conn_ = channel_manager_->AddChangeListener(std::bind(&Writer<MessageT>::OnChannelChange, this, std::placeholders::_1));

	// get peer readers
	const std::string& channel_name = this->role_attr_.channel_name();
	std::vector<proto::RoleAttributes> readers;
	//first time to sending message in the channel_id, readers must empty
	//if Shutdown() called,and restart the sending function, readers will not empty
	channel_manager_->GetReadersOfChannel(channel_name, &readers);
	for (auto& reader : readers) {
		transmitter_->Enable(reader);
	}
	//any receiver(reader) will be updated by Join function after OnChannelChange called 
